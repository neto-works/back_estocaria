...
namespace EstocariaNet.Services
{
    public class AdminsServices : IAdminServices
    {
        ...
        public async Task<AplicationUser> BuscarUserForAminByEmailAsync(string email)
        {
            AplicationUser? user = await _userManager.FindByEmailAsync(email);
            return (user is null) ? throw new ArgumentException($"Admin with this Email does not exist in the system.") : user;
        }
