...
namespace EstocariaNet.Controllers
{
    ...
    public class AdminsController : ControllerBase
    {
        ...
        [Authorize(Policy = "QuemPuderAdministrar")]
        [HttpGet("UserEmail/{emailAdmin}")]
        public async Task<IActionResult> GetUserAdminByEmail(string emailAdmin)
        {
            try
            {
                var user = await _adminServices.BuscarUserForAminByEmailAsync(emailAdmin);
                return Ok(user);
            }
            catch (ArgumentException a)
            {
